# OpenNotes - Active Tasks

**Active Epic**: [Test Coverage Improvement](epic-7a2b3c4d-test-improvement.md)  
**Overall Status**: ✅ READY TO START PHASE 1  
**Memory Status**: ✅ CLEANED UP (miniproject compliant)  
**Expected Completion**: 30 min (Phase 1) + 2 hours (Phase 2) + optional Phase 3

---

## TEST IMPROVEMENT EPIC - ACTIVE

### Phase 1: Critical Fixes ✅ COMPLETED
- [x] [task-8h9i0j1k-validate-path-tests.md](.memory/task-8h9i0j1k-validate-path-tests.md) - ValidatePath() tests ✅ (5 min)
- [x] [task-9i0j1k2l-template-error-tests.md](.memory/task-9i0j1k2l-template-error-tests.md) - Template error tests ✅ (ready for review)
- [x] [task-0j1k2l3m-db-context-tests.md](.memory/task-0j1k2l3m-db-context-tests.md) - DB context tests ✅ (15 min)

**Actual Outcome:** Coverage 73% → 84%+ (exceeded target!), all critical gaps fixed

### Phase 2: Core Improvements ✅ COMPLETED
- [x] Command error integration tests (17 min)
- [x] SearchNotes edge cases (12 min)
- [x] ObjectToFrontmatter edge cases (22 min)

**Actual Outcome:** Coverage significantly improved (testing 84%+), all critical gaps addressed

### Phase 3: Future-Proofing (2-3 hours, optional)
- [ ] Permission/filesystem error scenarios (30 min)
- [ ] Concurrency and race condition tests (60 min)
- [ ] Stress tests for large notebooks (60 min)

**Expected Outcome:** Coverage 80% → 85%+ (enterprise ready)

---

## Previous Epics (Archived)

### SQL Flag Feature
**Status**: ⏳ PENDING  
**Location**: `archive/02-sql-flag-feature/`  
**Note**: Ready for implementation after Test Improvement epic

---

## Quick Links

- **Epic Definition**: [epic-7a2b3c4d-test-improvement.md](epic-7a2b3c4d-test-improvement.md)
- **Navigation Guide**: [learning-8h9i0j1k-test-improvement-index.md](learning-8h9i0j1k-test-improvement-index.md)
- **Phase 1 Details**: [phase-3f5a6b7c-critical-fixes.md](phase-3f5a6b7c-critical-fixes.md)
- **Learning Files**: See summary.md for list

---

**Last Updated**: 2026-01-18 (Memory cleanup)
